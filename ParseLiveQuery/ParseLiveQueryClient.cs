...
namespace Parse.LiveQuery {
    public class ParseLiveQueryClient {
        ...
        public event Action<Exception> OnLiveQueryError;
        public event Action<Exception> OnSocketException;

        public static ParseLiveQueryClient Instance;
        ...
        private static Uri GetDefaultUri() {
            string server = ParseClient.Instance.Services.ServerConnectionData.ServerURI;
        ...
        private Task SendOperationWithSessionAsync(Func<string, IClientOperation> operationFunc) {
            return _taskQueue.EnqueueOnSuccess(
                ParseClient.Instance.CurrentUserController.GetCurrentSessionTokenAsync(ParseClient.Instance.Services, cancellationToken : CancellationToken.None),
        ...
        private void DispatchError(LiveQueryException exception) {
            ...
            OnLiveQueryError?.Invoke(exception);
        }
        ...
        private void HandleObjectEvent(Subscription.Event subscriptionEvent, IDictionary<string, object> jsonObject) {
            ...
            Subscription subscription;
            if (_subscriptions.TryGetValue(requestId, out subscription)) {
                ParseObject objState = (ParseObject)ParseClient.Instance.Services.Decoder.Decode(objectData, ParseClient.Instance.Services);
